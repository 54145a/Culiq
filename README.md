...
- **MCP client** — connect to remote Model Context Protocol servers (Streamable HTTP) in **Settings → MCP servers**; their tools are exposed to the agent as `server:tool` and toggle per server. A broken server surfaces as a `server:__connection_error` diagnostic tool instead of failing silently.
