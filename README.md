...
- **Defuddle markdown mode**: Defuddle's core bundle (`defuddle`) does not include Markdown conversion. Culiq uses `defuddle/full` for the `readDom` `markdown` mode, which imports `createMarkdownContent` separately. See [Defuddle #82](https://github.com/kepano/defuddle/issues/82).
- **Defuddle content removal**: Defuddle's scoring and selector removal can strip content from non-article pages (e.g. search results). Use `readable_html` mode with `contentSelector` for structured content.
